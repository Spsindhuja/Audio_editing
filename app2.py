...
# OpenAI Whisper API key should be set as environment variable
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")

def process_audio(audio_path, duration, volume=1.0, fade_in=2, fade_out=2, noise_reduction=True, reverb=False, echo=False):
    """Process the audio: volume normalization, fade-in/out, noise reduction."""
    if not os.path.exists(audio_path):
        raise FileNotFoundError(f"Audio file not found: {audio_path}")
        
    audio = AudioFileClip(audio_path).subclip(0, duration).fadein(fade_in).fadeout(fade_out).volumex(volume)
    ...
    if reverb:
        audio = audio.fx(vfx.audio_fadein, fade_in).fx(vfx.audio_fadeout, fade_out)
    if echo:
        audio = audio.fx(vfx.audio_fadein, fade_in).fx(vfx.audio_fadeout, fade_out)
    return audio

def generate_ai_subtitles(audio_path, language="en"):
    """Generate subtitles using OpenAI Whisper with optional language support."""
...
if __name__ == "__main__":
    ...
    audio_file = "input_audio.mp3"  # Replace with your audio file
    bg_image = "background.jpg"     # Replace with your background image
    output_file = "output_final.mp4"

    # Validate input files exist
    for file in [video_file, audio_file, bg_image]:
    ...
    if not OPENAI_API_KEY:
        print("Warning: OPENAI_API_KEY environment variable not set. AI subtitles will not work.")
